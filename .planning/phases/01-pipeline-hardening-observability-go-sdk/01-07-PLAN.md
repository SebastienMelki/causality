...
<task type="auto">
  ...
  <action>
    ...
    **Stateless Idempotent Design (no database state tracking needed):**
    Compaction is intentionally stateless — it does NOT use PostgreSQL or any persistent state to track which partitions have been compacted. Instead, idempotency is achieved through file-size inspection on every run:
    1. Each run lists files in each partition via S3 ListObjectsV2
    2. It filters to files smaller than `targetSize`
    3. If fewer than `minFiles` small files exist in a partition, it skips that partition (no-op)
    4. After a successful compaction, the partition contains a single large file — subsequent runs will find 0 small files and skip it
    5. This means re-running compaction on an already-compacted partition is a harmless no-op (reads file list, finds nothing to do, moves on)
    6. No compaction_runs table or state tracking is needed — the S3 file layout IS the state

    This design avoids the complexity of tracking compaction runs in a database, avoids stale state issues, and makes the service trivially restartable. The only cost is re-listing S3 objects on each run, which is cheap (one ListObjectsV2 call per partition).

    **Important safety rules:**
    - NEVER compact the current hour's partition (the hot partition being actively written to)
    - Compaction is idempotent: if a partition has no small files, it's a no-op (see above)
