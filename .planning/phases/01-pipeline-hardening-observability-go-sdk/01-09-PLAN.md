...
<task type="auto">
  ...
  <action>
    ...
    **Create `sdk/go/transport.go`:**
    ...
    Note: The SDK sends JSON, not protobuf. The HTTP server accepts protobuf via sebuf-generated handlers but should also accept JSON (check if the existing protobuf handlers support JSON content type via protojson). If not, the SDK may need to use the proto format. For simplicity and SDK independence, start with JSON. If the server only accepts protobuf, add a note that the server needs a JSON handler or the SDK should use protobuf encoding.

    Actually, checking the existing service.go — it uses sebuf-generated handlers which likely accept protojson. The SDK should send JSON that matches the proto message structure. The batch endpoint expects `IngestEventBatchRequest` with `events` field containing `EventEnvelope` messages. The JSON should match the protojson representation.
  </action>
  <verify>
...
- Transport sends to /v1/events/batch with X-API-Key header
